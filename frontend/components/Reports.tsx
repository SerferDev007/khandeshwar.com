...
export default function Reports({ transactions, onImportTransactions }: ReportsProps) {
  const { t } = useLanguage();
  ...
  const [exportFilters, setExportFilters] = useState<ExportFilterState>(initialExportFilterState);
  ...
  const getUniqueCategories = useMemo(() => {
    ...
    transactions.forEach(t => {
      if (filters.transactionType === 'all' || 
          ...
          (filters.transactionType === 'expenses' && (t.type === 'Expense' || t.type === 'Utilities' || t.type === 'Salary')) ||
  ...
  const getExportUniqueCategories = useMemo(() => {
    ...
    transactions.forEach(t => {
      if (exportFilters.transactionType === 'all' || 
          (exportFilters.transactionType === 'donations' && t.type === 'Donation') ||
          (exportFilters.transactionType === 'expenses' && (t.type === 'Expense' || t.type === 'Utilities' || t.type === 'Salary')) ||
          (exportFilters.transactionType === 'rentIncome' && t.type === 'RentIncome')) {
        categories.add(t.category);
      }
    });
    return Array.from(categories).sort();
    ...
    transactions.forEach(t => {
      if (t.subCategory && 
          (exportFilters.transactionType === 'all' || 
           ...
           (exportFilters.transactionType === 'expenses' && (t.type === 'Expense' || t.type === 'Utilities' || t.type === 'Salary')) ||
           (exportFilters.transactionType === 'rentIncome' && t.type === 'RentIncome')) &&
          (exportFilters.category === 'all' || t.category === exportFilters.category)) {
        subCategories.add(t.subCategory);
      }
    });
    return Array.from(subCategories).sort();
  ...
  const filteredTransactions = useMemo(() => {
    return transactions.filter(transaction => {
      // Type filter
      if (filters.transactionType !== 'all') {
        if (filters.transactionType === 'donations' && transaction.type !== 'Donation') return false;
        if (filters.transactionType === 'expenses' && !['Expense', 'Utilities', 'Salary'].includes(transaction.type)) return false;
        if (filters.transactionType === 'rentIncome' && transaction.type !== 'RentIncome') return false;
      }
      ...
      // SubCategory filter
      if (filters.subCategory !== 'all' && transaction.subCategory !== filters.subCategory) return false;
      ...
      if (filters.dateFilterType === 'month' && filters.month && filters.year) {
        ...
        if (transactionDate.getMonth() + 1 !== filterMonth || transactionDate.getFullYear() !== filterYear) {
          return false;
        }
      }
  ...
  const summaryStats = useMemo(() => {
    ...
    const expenses = filteredTransactions.filter(t => t.type === 'Expense' || t.type === 'Utilities' || t.type === 'Salary');
    const rentIncomes = filteredTransactions.filter(t => t.type === 'RentIncome');
  ...
  const handleExportFilterChange = useCallback((key: keyof ExportFilterState, value: string) => {
  ...
  const hasActiveFilters = useMemo(() => {
    return filters.transactionType !== 'all' ||
           ...
           (filters.dateFilterType === 'range' && (filters.fromDate || filters.toDate));
  }, [filters]);

  // Check if any export filters are applied
  const hasActiveExportFilters = useMemo(() => {
    return exportFilters.transactionType !== 'all' ||
           exportFilters.category !== 'all' ||
           exportFilters.subCategory !== 'all' ||
           exportFilters.dateFilterType !== 'all' ||
           (exportFilters.dateFilterType === 'month' && (exportFilters.month || exportFilters.year)) ||
           (exportFilters.dateFilterType === 'range' && (exportFilters.fromDate || exportFilters.toDate));
  }, [exportFilters]);
  ...
  const getFilteredTransactions = () => {
    ...
    switch (exportFilters.transactionType) {
      case 'donations':
        ...
        break;
      case 'expenses':
        filtered = filtered.filter(t => t.type === 'Expense' || t.type === 'Utilities' || t.type === 'Salary');
        break;
      case 'rentIncome':
    ...
    if (exportFilters.subCategory !== 'all') {
      filtered = filtered.filter(t => t.subCategory === exportFilters.subCategory);
    }

    // Date filters
    const transactionDate = (dateStr: string) => new Date(dateStr);
    
    if (exportFilters.dateFilterType === 'month' && exportFilters.month && exportFilters.year) {
  ...
  const drawTable = (doc: jsPDF, startY: number, headers: string[], data: string[][], options: {
    headerColor?: number[];
    cellPadding?: number;
    fontSize?: number;
    columnWidths?: number[];
    ...
    data.forEach((row, rowIndex) => {
      ...
      row.forEach((cell, colIndex) => {
        ...
        if (colIndex === headers.length - 1 && cellText.includes('₹')) {
          doc.text(finalText, x + colWidths[colIndex] - cellPadding, currentY + rowHeight - cellPadding, { align: 'right' });
        } else {
          doc.text(finalText, x + cellPadding, currentY + rowHeight - cellPadding);
        }
      });
  ...
  const generatePDF = (reportType: string, language: 'en' | 'mr') => {
    ...
    doc.setFontSize(16);
    if (language === 'mr') {
      doc.text('श्री क्षेत्र खंडेश्वर देवस्थान कुसळंब', pageWidth / 2, yPosition, { align: 'center' });
    } else {
      doc.text(getTranslation('header.title', language), pageWidth / 2, yPosition, { align: 'center' });
    }
    yPosition += 10;
    
    doc.setFontSize(12);
    doc.text(getTranslation('header.subtitle', language), pageWidth / 2, yPosition, { align: 'center' });
    yPosition += 20;
    ...
    doc.setFontSize(10);
    doc.text(`${getTranslation('reports.generatedOn', language)}: ${new Date().toLocaleDateString()}`, 20, yPosition);
    yPosition += 10;
    ...
    if (hasActiveExportFilters) {
      ...
      if (exportFilters.dateFilterType === 'month' && exportFilters.month && exportFilters.year) {
        const monthName = new Date(parseInt(exportFilters.year), parseInt(exportFilters.month) - 1, 1)
          .toLocaleString('default', { month: 'long', year: 'numeric' });
        doc.text(`- Month: ${monthName}`, 25, yPosition);
        yPosition += 5;
      }
      if (exportFilters.dateFilterType === 'range' && (exportFilters.fromDate || exportFilters.toDate)) {
    ...
    // Period
    if (exportFilters.fromDate || exportFilters.toDate || exportFilters.dateFilterType !== 'all') {
      let periodText = '';
      if (exportFilters.dateFilterType === 'month' && exportFilters.month && exportFilters.year) {
        const monthName = new Date(parseInt(exportFilters.year), parseInt(exportFilters.month) - 1, 1)
          .toLocaleString('default', { month: 'long', year: 'numeric' });
        periodText = `${getTranslation('reports.period', language)}: ${monthName}`;
      } else {
        const fromDate = exportFilters.fromDate || 'Start';
        const toDate = exportFilters.toDate || 'End';
        periodText = `${getTranslation('reports.period', language)}: ${fromDate} to ${toDate}`;
      }
  ...
  const generateTransactionPDF = (doc: jsPDF, startY: number, language: 'en' | 'mr') => {
    ...
    const tableData = filtered.map((t, index) => [
      (index + 1).toString(),
      t.date,
      t.type === 'RentIncome' ? (language === 'mr' ? 'भाडे उत्पन्न' : 'Rent Income') : 
      t.type === 'Donation' ? getTranslation('reports.donations', language) : 
    ...
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(10);
    doc.text(`${getTranslation('reports.totalAmount', language)}: ₹${filtered.reduce((sum, t) => sum + t.amount, 0).toLocaleString()}`, 20, finalY + 5);
  };

  const generateSummaryPDF = (doc: jsPDF, startY: number, language: 'en' | 'mr') => {
    const filtered = getFilteredTransactions();
    const donations = filtered.filter(t => t.type === 'Donation');
    const expenses = filtered.filter(t => t.type === 'Expense' || t.type === 'Utilities' || t.type === 'Salary');
    ...
    const headers = [getTranslation('reports.overview', language), getTranslation('common.amount', language)];
    const summaryData = [
      [getTranslation('dashboard.totalDonations', language), `₹${totalDonations.toLocaleString()}`],
      [getTranslation('dashboard.totalRentIncome', language), `₹${totalRentIncome.toLocaleString()}`],
      [getTranslation('dashboard.totalExpenses', language), `₹${totalExpenses.toLocaleString()}`],
      [getTranslation('dashboard.netBalance', language), `₹${netBalance.toLocaleString()}`]
    ];
  ...
  const generateCategoryPDF = (doc: jsPDF, startY: number, language: 'en' | 'mr') => {
    const filtered = getFilteredTransactions();
    const donations = filtered.filter(t => t.type === 'Donation');
    const expenses = filtered.filter(t => t.type === 'Expense' || t.type === 'Utilities' || t.type === 'Salary');
  ...
  const generateMonthlyPDF = (doc: jsPDF, startY: number, language: 'en' | 'mr') => {
    const filtered = getFilteredTransactions();
    ...
    const monthlyTableData = Object.entries(monthlyData).map(([month, data]) => [
      ...
      `₹${(data.donations + data.rentIncome - data.expenses).toLocaleString()}`
    ]);
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (exportFilters.exportFormat === 'pdf') {
        const doc = generatePDF(exportFilters.exportType, exportFilters.pdfLanguage as 'en' | 'mr');
        const filename = `${exportFilters.exportType}_${exportFilters.pdfLanguage === 'mr' ? 'marathi' : 'english'}_${timestamp}.pdf`;
        doc.save(filename);
        ...
        switch (exportFilters.exportType) {
          case 'transactions': {
            const headers = ['Date', 'Type', 'Category', 'SubCategory', 'Description', 'Amount'];
          ...
          case 'summary': {
            ...
            const data = [
              ...
              { Metric: t('dashboard.totalRentIncome'), Amount: totalRentIncome },
          ...
          case 'monthly': {
            ...
            const headers = ['Month', 'Donations', 'RentIncome', 'Expenses', 'Net'];
  ...
  const parseCSV = (csvContent: string): Transaction[] => {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const validTypes = ['Donation', 'Expense', 'Utilities', 'Salary', 'RentIncome'];
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      toast.success(`${t('reports.importSuccess')} (${importedTransactions.length} ${t('reports.totalTransactions').toLowerCase()})`);
              ...
              <div className="space-y-6 max-h-96 overflow-y-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <Select 
                      ...
                      onValueChange={(value) => handleExportFilterChange('exportFormat', value)}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                  ...
                  <div>
                    ...
                    <Select 
                      ...
                      onValueChange={(value) => handleExportFilterChange('exportType', value)}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="transactions">{t('reports.exportTransactions')}</SelectItem>
                        <SelectItem value="summary">{t('reports.exportSummary')}</SelectItem>
                        <SelectItem value="categoryBreakdown">{t('reports.exportCategorySummary')}</SelectItem>
                        <SelectItem value="monthly">{t('reports.monthlyTrend')}</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                </div>
                ...
                {exportFilters.exportFormat === 'pdf' && (
                  <div>
                    ...
                    <Select 
                      ...
                      onValueChange={(value) => handleExportFilterChange('pdfLanguage', value)}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="en">{t('reports.englishPDF')}</SelectItem>
                        <SelectItem value="mr">{t('reports.marathiPDF')}</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                )}
                ...
                <div className="border-t pt-4">
                  <div className="flex items-center justify-between mb-4">
                    <h4 className="font-medium text-sm">{t('reports.filters')}</h4>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <Select
                        ...
                        onValueChange={(value) => handleExportFilterChange('transactionType', value)}
                      >
                        <SelectTrigger>
                        ...
                        <SelectContent>
                          ...
                          <SelectItem value="donations">{t('reports.donations')}</SelectItem>
                          <SelectItem value="expenses">{t('reports.expenses')}</SelectItem>
                    ...
                    <div>
                      ...
                      <Select
                        ...
                        onValueChange={(value) => handleExportFilterChange('category', value)}
                      >
                        <SelectTrigger>
                    ...
                    <div>
                      ...
                      <Select
                        ...
                        onValueChange={(value) => handleExportFilterChange('subCategory', value)}
                      ...
                      >
                        ...
                        <SelectContent>
                          <SelectItem value="all">{t('reports.allSubCategories')}</SelectItem>
                    ...
                    <div>
                      ...
                      <Select
                        ...
                        onValueChange={(value) => handleExportFilterChange('dateFilterType', value)}
                      >
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                          ...
                          <SelectItem value="month">{t('reports.filterByMonth')}</SelectItem>
                          <SelectItem value="range">{t('reports.filterByDateRange')}</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                  </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                      <div>
                        ...
                        <Select
                          ...
                          onValueChange={(value) => handleExportFilterChange('month', value)}
                        >
                          <SelectTrigger>
                      ...
                      <div>
                        ...
                        <Select
                          ...
                          onValueChange={(value) => handleExportFilterChange('year', value)}
                        >
                          <SelectTrigger>
                  ...
                  {hasActiveExportFilters && (
                    <div className="pt-2 border-t mt-4">
                      <div className="flex flex-wrap gap-2">
                        {exportFilters.transactionType !== 'all' && (
                          <Badge variant="secondary" className="flex items-center gap-1">
                            ...
                            <X 
                              ...
                              onClick={() => handleExportFilterChange('transactionType', 'all')}
                            />
                          </Badge>
                        )}
                        {exportFilters.category !== 'all' && (
                          <Badge variant="secondary" className="flex items-center gap-1">
                            ...
                            <X 
                              className="h-3 w-3 cursor-pointer" 
                              onClick={() => handleExportFilterChange('category', 'all')}
                            />
                          </Badge>
                        )}
                        {exportFilters.subCategory !== 'all' && (
                          <Badge variant="secondary" className="flex items-center gap-1">
                            ...
                            <X 
                              className="h-3 w-3 cursor-pointer" 
                              onClick={() => handleExportFilterChange('subCategory', 'all')}
                            />
                          </Badge>
                        )}
                        {exportFilters.dateFilterType === 'month' && exportFilters.month && exportFilters.year && (
                          <Badge variant="secondary" className="flex items-center gap-1">
                            Month: {new Date(parseInt(exportFilters.year), parseInt(exportFilters.month) - 1, 1).toLocaleString('default', { month: 'long', year: 'numeric' })}
                            <X 
                              className="h-3 w-3 cursor-pointer" 
                              onClick={() => {
                        ...
                        {exportFilters.dateFilterType === 'range' && (exportFilters.fromDate || exportFilters.toDate) && (
                          <Badge variant="secondary" className="flex items-center gap-1">
                            Date Range: {exportFilters.fromDate || 'Start'} to {exportFilters.toDate || 'End'}
                            <X 
                              className="h-3 w-3 cursor-pointer" 
                              onClick={() => {
                ...
                <div className="bg-gray-50 p-3 rounded-md">
                  ...
                  {hasActiveExportFilters && (
                    <div className="text-xs text-blue-600 mt-1">
                      Filters applied - results will be limited to matching records
                    </div>
                  )}
                </div>
              ...
              <DialogContent className="max-w-md">
                ...
                <div className="space-y-4">
                  ...
                  <Alert>
                    ...
                    <AlertDescription>
                      <div className="space-y-1">
                        ...
                        <div className="text-xs">{t('reports.sampleFormat')}</div>
                      </div>
                    </AlertDescription>
                  </Alert>
                  ...
                  <Button 
                    ...
                    className="w-full"
                  >
                    {importing ? t('reports.processingImport') : t('reports.import')}
                  </Button>
                </div>
              </DialogContent>
            </Dialog>
            ...
            {hasActiveFilters && (
              <div className="pt-2 border-t">
                <div className="flex flex-wrap gap-2">
                  ...
                  {filters.dateFilterType === 'month' && filters.month && filters.year && (
                    <Badge variant="secondary" className="flex items-center gap-1">
                      Month: {new Date(parseInt(filters.year), parseInt(filters.month) - 1, 1).toLocaleString('default', { month: 'long', year: 'numeric' })}
                      <X 
                        className="h-3 w-3 cursor-pointer" 
                        onClick={() => {
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Select value={viewMode} onValueChange={(value: 'table' | 'summary') => setViewMode(value)}>
              <SelectTrigger className="w-32">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="table">{t('reports.tableView')}</SelectItem>
                <SelectItem value="summary">{t('reports.summaryView')}</SelectItem>
              </SelectContent>
            </Select>
          </div>
        </div>
              ...
              <div className="flex items-center justify-between">
                <div className="text-sm text-gray-600">
                  {hasActiveFilters ? 
                    ...
                    }).replace('{count}', filteredTransactions.length.toString()).replace('{total}', transactions.length.toString()) :
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <Card>
                ...
                <CardContent>
                  ...
                  <p className="text-xs text-gray-500">
                    {filteredTransactions.filter(t => t.type === 'Donation').length} transactions
                  </p>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  <p className="text-xs text-gray-500">
                    {filteredTransactions.filter(t => ['Expense', 'Utilities', 'Salary'].includes(t.type)).length} transactions
                  </p>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  <p className="text-xs text-gray-500">
                    {filteredTransactions.filter(t => t.type === 'RentIncome').length} transactions
                  </p>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent>
                  <div className={`text-2xl font-bold ${summaryStats.netBalance >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {Object.entries(summaryStats.monthlyData)
                        ...
                        .map(([month, data]) => {
                          const net = data.donations + data.rentIncome - data.expenses;
                          return (
