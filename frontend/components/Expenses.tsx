...
export default function Expenses({ expenses, onAddExpense }: ExpensesProps) {
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.payeeContact.trim() && !/^\d{10}$/.test(formData.payeeContact.trim())) {
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="Gala Kharch">{t('expenses.galaKharch')}</SelectItem>
                    <SelectItem value="Mandir Dekhbhal">{t('expenses.mandirDekhbhal')}</SelectItem>
                  </SelectContent>
                </Select>
                {errors.category && (
                  <p className="text-sm text-red-500 mt-1 flex items-center gap-1">
              ...
              {formData.category && (
                <div>
                  ...
                  >
                    ...
                    <SelectContent>
                      {expenseCategorySubCategories[formData.category as keyof typeof expenseCategorySubCategories]?.map((subCat) => (
              ...
              {/* Payee Contact */}
              <div>
                <Label htmlFor="payeeContact">{t('expenses.payeeContact')}</Label>
                <Input
                  id="payeeContact"
              ...
              {/* Amount */}
              <div>
                <Label htmlFor="amount">{t('common.amount')} ({t('common.currency')}) *</Label>
                <Input
                  id="amount"
                  type="text"
            ...
            <div className="mt-4 p-4 bg-gray-50 rounded-lg border">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{formatDate(lastAddedExpense.date)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">{t('donations.category')}:</span>
                  <span className="font-medium">
                    {lastAddedExpense.category === 'Utsav' && t('expenses.utsav')}
                    {lastAddedExpense.category === 'Gala Kharch' && t('expenses.galaKharch')}
                    {lastAddedExpense.category === 'Mandir Dekhbhal' && t('expenses.mandirDekhbhal')}
                  </span>
                </div>
                {lastAddedExpense.subCategory && (
                  <div className="flex justify-between">
                    <span className="text-gray-600">{t('donations.subCategory')}:</span>
                    <span className="font-medium">{t(`expenses.${lastAddedExpense.subCategory}`)}</span>
                  </div>
                )}
                <div className="flex justify-between">
                  <span className="text-gray-600">{t('expenses.payeeName')}:</span>
                  <span className="font-medium">{lastAddedExpense.payeeName}</span>
                </div>
                {lastAddedExpense.payeeContact && (
                  <div className="flex justify-between">
                    <span className="text-gray-600">{t('expenses.payeeContact')}:</span>
                    <span className="font-medium">{lastAddedExpense.payeeContact}</span>
                  </div>
                )}
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  <span className="text-gray-600">{t('expenses.details')}:</span>
                  <span className="font-medium">{lastAddedExpense.description}</span>
                </div>
              </div>
            </div>
          )}
      ...
      <Card>
        ...
        <CardContent>
          <div className="mb-4">
            <p className="text-lg">{t('dashboard.totalExpenses')}: <span className="text-red-600">{formatCurrency(totalExpenses)}</span></p>
          </div>
